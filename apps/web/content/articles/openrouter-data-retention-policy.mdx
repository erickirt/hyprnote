...
![An OpenRouter request passes through the gateway, then the underlying provider's own retention policy applies](/images/blog/articles/openrouter-data-retention-policy/openrouter-request-path.png)
...
Anarlog supports OpenRouter as an API provider option. When you bring your own OpenRouter API key, your meeting data routes through OpenRouter under your account settings.

If you configure OpenRouter with ZDR enabled and prompt logging off, those settings apply to requests Anarlog sends through your account. If you have locked routing to a specific provider within OpenRouter, requests from Anarlog will follow that configuration.
...
[Download Anarlog for MacOS](https://anarlog.so/download) and use the AI provider your security team actually approves.